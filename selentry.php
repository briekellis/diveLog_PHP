...
if (!$_POST)  {
	...
	$get_master_sql = "SELECT concat_ws(' ', f_name, l_name) as display_name
	                   FROM diver_name WHERE id = '".$safe_id."'";
	...
	if (mysqli_num_rows($get_tel_res) > 0) {
		...
		$display_block .= "<p><strong>Dive Buddy:</strong><br/>
		<ul>";
	...
	$display_block .= "<br/>
	<p style=\"text-align: center\"><a href=\"addentry.php?master_id=".$_POST['sel_id']."\">add info</a> ...<a href=\"".$_SERVER['PHP_SELF']."\">select another</a>...<a href='addressBookMenu.html'>main menu</a></p>";
}
